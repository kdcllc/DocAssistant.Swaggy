...
{
    public class CustomSaveRecordsHandler : IPipelineStepHandler
    {
        ...
        private readonly ILogger<CustomSaveRecordsHandler> _log;
        ...
        public CustomSaveRecordsHandler(
            string stepName,
            IPipelineOrchestrator orchestrator,
            ILogger<CustomSaveRecordsHandler>? log = null)
        {
            this.StepName = stepName;
            this._log = log ?? DefaultLogger<CustomSaveRecordsHandler>.Instance;
        ...
        {
            IndexCreationInformation.IndexCreationInfo.StepInfo = $"{StepName}: Copying embeddings from storage to list of memory DBs.";
        ...
        public async Task<(bool success, DataPipeline updatedPipeline)> SaveEmbeddingsAsync(
            DataPipeline pipeline, CancellationToken cancellationToken = default)
        {
            var embeddingFiles = GetListOfEmbeddingFiles(pipeline).ToList();

            IndexCreationInformation.IndexCreationInfo.Value = 0;
            IndexCreationInformation.IndexCreationInfo.Max = embeddingFiles.Count;
            // For each embedding file => For each Memory DB => Upsert record
            foreach (FileDetailsWithRecordId file in embeddingFiles)
            {
